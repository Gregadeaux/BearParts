...
/** Live, filterable, grouped task list. */
export function TasksView({ initialTasks, team, subgroups, allTags, userId, openTaskId }: Props) {
  ...
  const groups = useMemo(
    () => groupTasks(filterTasks(tasks, filters, userId), groupBy, subgroups),
    [tasks, filters, userId, groupBy, subgroups],
  );
  ...
  const closeDialog = (open: boolean) => {
    ...
    if (openTaskId) router.replace("/tasks");
  };
  ...
  const quickAdd = async (title: string, defaults: DialogState["defaults"]) => {
    try {
      await createTaskAction(
        { title, status: defaults?.status, subgroupId: defaults?.subgroupId ?? null },
        [],
        [],
      );
      refetch();
  ...
  return (
    <div className="space-y-3">
      <TaskFilters
        ...
        subgroups={subgroups}
        allTags={allTags}
        onNewTask={() => setDialog({ open: true, task: null, defaults: {} })}
      />
